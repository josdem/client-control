...
/**
 * 
 * Esta clase no debe de ser usada en producci�n. ya que su implementacion no
...
public final class BogusSslContextFactory {
	 ...
	 */
	private static final String BOGUS_KEYSTORE_1 = "bogus.cert";
	private static final String BOGUS_KEYSTORE_2 = "/bogus.cert";
	...
	// keytool -genkey -alias bogus -keysize 512 -validity 3650 -keyalg RSA -dname
	// "CN=bogus.com, OU=XXX CA, O=Bogus Inc, L=Stockholm, S=Stockholm, C=SE"
	// -keypass boguspw -storepass boguspw
	// -keystore bogus.cert
	...
	private static SSLContext createBougusServerSslContext() throws GeneralSecurityException, IOException {
		...
		InputStream in = null;
		try {
			in = BogusSslContextFactory.class.getResourceAsStream(BOGUS_KEYSTORE_1);
			if (in == null) {
				in = BogusSslContextFactory.class.getResourceAsStream(BOGUS_KEYSTORE_2);
				if (in == null) {
					throw new GeneralSecurityException("Unable to find key!");
				}
			}
			ks.load(in, BOGUS_PW);
		} finally {
		...
		sslContext.init(kmf.getKeyManagers(), new BogusTrustManagerFactory().getTrustManagers(), null);
	...
	private static SSLContext createBougusClientSslContext() throws GeneralSecurityException {
		...
		context.init(null, new BogusTrustManagerFactory().getTrustManagers(), null);
		return context;
	}
}
